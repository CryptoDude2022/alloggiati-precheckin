  ...
  try {
    ...
    for (let i = 0; i < guests.length; i++) {
      ...
      const line = [
        ...
        pad(clean(g.statoNascita) || "", 9),                                // Stato Nascita cod. (9) - usa codice fornito dal form
        pad(clean(g.cittadinanza) || "", 9),                                // Cittadinanza cod. (9) - usa codice fornito dal form
    ...
    // Sesso: deve essere "1" (maschio) o "2" (femmina) come da specifica XML
    ...
    for (let i = 0; i < guests.length; i++) {
      ...
      // Stato di nascita: usa il codice ISTAT dal form (già presente in g.statoNascita)
      let statoNascita = clean(g.statoNascita) || "";
      if (!statoNascita) {
        try {
...
<movimenti>
  ...
  <movimento>
    ...
    <struttura>
      ...
      <dataarrivo>${dataArrivoGIES}</dataarrivo>
    </struttura>
    <arrivi>${xmlArrivi}
    </arrivi>
...
`;
    ...
    if (emailOspite) {
      emailPayload.replyTo = emailOspite;
    }
